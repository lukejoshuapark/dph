...
func ProcessFlag(ctx context.Context, posthogService posthog.Service, cfg *config.Config, flag *schema.Flag, dryRun bool) error {
	...
	if existingFlag == nil {
		return createNewFlag(ctx, posthogService, cfg, flag, dryRun)
	} else {
		return updateExistingFlag(ctx, posthogService, cfg, flag, existingFlag, dryRun)
	}
}

func createNewFlag(ctx context.Context, posthogService posthog.Service, cfg *config.Config, flag *schema.Flag, dryRun bool) error {
	if dryRun {
	...
	if err := posthogService.CreateFlag(ctx, cfg.ProjectId, requestModel); err != nil {
